...
namespace Paulsams.SearchableAttributeDrawer.Editor
{
    public class SearchableAdvancedDropdownWindow : AdvancedDropdown
    {
        ...
        private static readonly float _padding =
            (EditorGUIUtility.singleLineHeight + EditorGUIUtility.standardVerticalSpacing) * 2f + 10f;
        ...
        private static void SetNextChild(AdvancedDropdownState state, Type stateType,
            object[] parametersFromGetStateForItem, Item nextItem)
        {
            parametersFromGetStateForItem[0] = nextItem.DropdownItem;
            stateType.GetMethod("GetStateForItem", BindingFlags.Instance | BindingFlags.NonPublic)
                .Invoke(state, parametersFromGetStateForItem);
        }

        private static void SetScroll(AdvancedDropdownState state, Type stateType, Item item,
            object[] parametersFromSetScrollState, Item nextItem)
        {
            parametersFromSetScrollState[0] = item.DropdownItem;
            parametersFromSetScrollState[1] = new Vector2(0f,
                _settings.HeightItem * (nextItem.Index - SettingsForSearchableWindow.HalfShowedElements + 0.5f));
            stateType.GetMethod("SetScrollState", BindingFlags.Instance | BindingFlags.NonPublic)
                .Invoke(state, parametersFromSetScrollState);
        }

        private static void SetIndex(AdvancedDropdownState state, Type stateType, Item item,
            object[] parametersFromSelectedIndex, Item nextItem)
        {
            parametersFromSelectedIndex[0] = item.DropdownItem;
            parametersFromSelectedIndex[1] = nextItem.Index;
            stateType.GetMethod("SetSelectedIndex", BindingFlags.Instance | BindingFlags.NonPublic)
                .Invoke(state, parametersFromSelectedIndex);
        }
